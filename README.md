...
In-progress work using real background-run camera images (via `real_data_loading.py`) to check/refine the rendering pipeline's camera pose estimates against the physical detector lives on the `camera-position-optimization` branch, not yet merged here.
